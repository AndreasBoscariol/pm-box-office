...
def cmd_ingest_theatres(args: Any) -> int:
    ...
    try:
        db.initialize_amc_database(conn)
        xml_text, cache_path = fetch_theatre_sitemap(fetcher)
        theatres = parse_theatre_sitemap(xml_text)
        count = db.upsert_theatres(conn, theatres)
        run_id = db.create_collection_run(
            conn,
            run_type="sitemap",
            selected_theatre_ids=[theatre.amc_theatre_id for theatre in theatres],
            status="completed",
        )
        ...
        conn.close()
    print(f"Upserted {count} AMC theatres from {cache_path}.")
    return 0
