#!/usr/bin/env python3
"""Ingest Edward Douglas Substack prediction posts into PostgreSQL."""
...
BASE_URL = "https://edwarddouglas.substack.com"
FEED_URL = f"{BASE_URL}/feed"
ARCHIVE_API_URL = f"{BASE_URL}/api/v1/archive"
DEFAULT_CACHE_DIR = Path("data/raw/edwarddouglas_substack")
...
DEFAULT_USER_AGENT = "pm-box-office-edwarddouglas-substack-bot/1.0 (+personal research; set --user-agent contact)"
MIN_DELAY_SECONDS = 5.0
PARSER_VERSION = "edwarddouglas_substack_predictions_v2"
DEFAULT_PER_PAGE = 20
...
def parse_rss_posts(xml_text: str, *, source_url: str) -> list[theory.PostRecord]:
    try:
        ...
        raise ValueError(f"invalid Edward Douglas Substack RSS from {source_url}: {exc}") from exc
    ...
    for item in root.findall("./channel/item"):
        ...
        posts.append(
            theory.PostRecord(
                ...
                author=text_or_none(item.find("dc:creator", RSS_NAMESPACES)) or "Edward Douglas",
...
def parse_archive_items(raw_json: str, *, source_url: str) -> list[dict[str, Any]]:
    try:
        ...
        raise ValueError(f"invalid Edward Douglas Substack archive JSON from {source_url}: {exc}") from exc
    if not isinstance(payload, list):
        raise ValueError(f"expected list from Edward Douglas Substack archive: {source_url}")
...
def parse_post_record_json(raw_json: str, *, source_url: str) -> theory.PostRecord:
    try:
        ...
        raise ValueError(f"invalid Edward Douglas Substack post JSON from {source_url}: {exc}") from exc
    if not isinstance(payload, dict):
        raise ValueError(f"expected object from Edward Douglas Substack post: {source_url}")
    ...
    return theory.PostRecord(
        ...
        author=published_author(payload) or "Edward Douglas",
        published_at=published_at.isoformat(),
        published_date=published_at.date().isoformat(),
        excerpt=theory.strip_html_text(description) or None,
...
def parse_predictions(post: theory.PostRecord) -> list[theory.TheoryPrediction]:
    ...
    parser = theory.PredictionHtmlParser()
    parser.feed(post.content_html)
    weekend_predictions = parse_weekend_warrior_predictions(
        parser.blocks,
        post=post,
        first_row_ordinal=len(predictions) + 1,
    )
    predictions.extend(theory.dedupe_predictions(weekend_predictions, existing=predictions))
    return [retag_prediction(prediction) for prediction in predictions]


def parse_weekend_warrior_predictions(
    blocks: list[str],
    *,
    ...
    active = False
    for block in blocks:
        lines = [theory.clean_text(line) for line in block.split("\n") if theory.clean_text(line)]
        if not lines:
            continue
        heading_date = weekend_prediction_heading_date(lines[0], post.published_date)
        if heading_date is not None:
            forecast_date = heading_date
            active = True
            lines = lines[1:]
        elif prediction_section_starts(lines[0]):
            active = True
            lines = lines[1:]
        if not active:
            continue
        for line in lines:
            if prediction_section_starts(line):
                maybe_date = weekend_prediction_heading_date(line, post.published_date)
                forecast_date = maybe_date or forecast_date
                continue
            prediction = parse_weekend_warrior_prediction_line(
                line,
                post=post,
        ...
        if predictions and not any(weekend_prediction_line_prefix(line) for line in lines):
            active = False
    return predictions


def prediction_section_starts(value: str) -> bool:
    return "box office predictions" in value.lower()


def weekend_prediction_heading_date(value: str, published_date: str) -> str | None:
    if not prediction_section_starts(value):
        return None
    match = re.search(
        r"\b(?:for\s+)?([A-Za-z]+\.?\s+\d{1,2},\s*\d{4}|\d{1,2}/\d{1,2}/\d{2,4})\b",
        value,
        flags=re.IGNORECASE,
    )
    if not match:
        return None
    return coerce_forecast_weekend_date(theory.parse_release_date(match.group(1), published_date), published_date)


def weekend_prediction_line_prefix(value: str) -> bool:
    return re.match(r"^\s*(?:\d{1,2}\.|[-\u2013\u2014])\s+", value) is not None


def parse_weekend_warrior_prediction_line(
    line: str,
    *,
    post: theory.PostRecord,
    forecast_date: str | None,
    row_ordinal: int,
) -> theory.TheoryPrediction | None:
    if not weekend_prediction_line_prefix(line):
        return None
    normalized = line.replace("\u2013", "-").replace("\u2014", "-")
    match = re.match(r"^\s*(?:\d{1,2}\.|-)\s+(.+?)\s+-\s+(\$.*)$", normalized)
    if not match:
        return None
    title_text = theory.clean_text(match.group(1))
    forecast_text = theory.clean_text(match.group(2))
    weekend_forecast = theory.first_money(forecast_text)
    if weekend_forecast is None:
        return None
    title, distributor = parse_weekend_warrior_title(title_text)
    if not title:
        return None
    if forecast_date is None:
        forecast_date = post_title_forecast_date(post.title, post.published_date)
    return theory.build_prediction(
        post=post,
        title=title,
        distributor=distributor,
        release_date=forecast_date,
        prediction_scope="weekend_forecast",
        forecast_metric="domestic_weekend",
        weekend_forecast_usd=weekend_forecast,
        weekend_day_count=3,
        percent_change=theory.parse_percent_change(forecast_text),
        change_label=parse_weekend_warrior_change_label(forecast_text),
        raw_forecast_text=line,
        source_context="weekend_warrior_prediction_list",
        row_ordinal=row_ordinal,
    )


def post_title_forecast_date(value: str, published_date: str) -> str | None:
    match = re.search(
        r"\b(?:for\s+)?([A-Za-z]+\.?\s+\d{1,2},\s*\d{4}|\d{1,2}/\d{1,2}/\d{2,4})\b",
        value,
        flags=re.IGNORECASE,
...
def coerce_forecast_weekend_date(value: str | None, published_date: str) -> str | None:
    if value is None:
        return None
    try:
        ...
        return value
    if parsed < published - dt.timedelta(days=180):
        same_year = parsed.replace(year=published.year)
        if published - dt.timedelta(days=14) <= same_year <= published + dt.timedelta(days=14):
            return same_year.isoformat()
    return value


def parse_weekend_warrior_title(value: str) -> tuple[str | None, str | None]:
    match = re.match(r"(.+?)\s*\(([^()]*)\)\s*$", value)
    if match:
        return theory.clean_movie_title(match.group(1)), theory.clean_text(match.group(2)) or None
    return theory.clean_movie_title(value), None


def parse_weekend_warrior_change_label(value: str) -> str | None:
    cleaned = re.sub(r"^\$\s*\d[\d,]*(?:\.\d+)?\s*(?:million|billion|[kmbKMB])?", "", value, flags=re.IGNORECASE)
    cleaned = theory.clean_text(cleaned)
...
def retag_prediction(prediction: theory.TheoryPrediction) -> theory.TheoryPrediction:
    ...
    return replace(
        prediction,
        source_movie_id=edwarddouglas_source_movie_id(
            normalized_movie_title=prediction.normalized_movie_title,
            release_date=prediction.release_date,
            prediction_scope=prediction.prediction_scope,
            published_date=prediction.prediction_made_date,
        ),
...
def edwarddouglas_source_movie_id(
    *,
    ...
    if prediction_scope == "weekend_forecast":
        return f"edwarddouglas_substack:{theory.DOMESTIC_MARKET}:{slug}"
    date_key = release_date or published_date
    return f"edwarddouglas_substack:{theory.DOMESTIC_MARKET}:{slug}:{date_key}"
...
def initialize_database(conn: Any) -> None:
    ...
    conn.executescript(
        """
        CREATE TABLE IF NOT EXISTS edwarddouglas_substack_posts (
        ...
        CREATE TABLE IF NOT EXISTS edwarddouglas_substack_predictions (
            prediction_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            post_id BIGINT NOT NULL REFERENCES edwarddouglas_substack_posts(post_id),
        ...
        ALTER TABLE edwarddouglas_substack_predictions
            ADD COLUMN IF NOT EXISTS movie_id BIGINT REFERENCES movies(movie_id);

        CREATE TABLE IF NOT EXISTS edwarddouglas_substack_ingest_issues (
        ...
        CREATE INDEX IF NOT EXISTS idx_edwarddouglas_substack_predictions_movie_id
            ON edwarddouglas_substack_predictions(movie_id);
        CREATE INDEX IF NOT EXISTS idx_edwarddouglas_substack_predictions_title
            ON edwarddouglas_substack_predictions(normalized_movie_title);
        CREATE INDEX IF NOT EXISTS idx_edwarddouglas_substack_predictions_release_date
            ON edwarddouglas_substack_predictions(release_date);
        """
    )
...
def upsert_post(
    ...
    *,
    ...
    conn.execute(
        """
        INSERT INTO edwarddouglas_substack_posts (
        ...
        ON CONFLICT(source_post_id) DO UPDATE SET
            ...
            fetched_at = COALESCE(excluded.fetched_at, edwarddouglas_substack_posts.fetched_at),
            raw_cache_path = COALESCE(excluded.raw_cache_path, edwarddouglas_substack_posts.raw_cache_path),
            sha256 = COALESCE(excluded.sha256, edwarddouglas_substack_posts.sha256),
    ...
    return int(
        conn.execute(
            "SELECT post_id FROM edwarddouglas_substack_posts WHERE source_post_id = %s",
            (post.source_post_id,),
        ).fetchone()[0]
    )
...
def insert_predictions(
    ...
    *,
    ...
    conn.executemany(
        """
        INSERT INTO edwarddouglas_substack_predictions (
...
def insert_issue(
    ...
    *,
    ...
    conn.execute(
        """
        INSERT INTO edwarddouglas_substack_ingest_issues (
...
def clear_post_issues(conn: Any, *, issue_source: str, post_url: str) -> None:
    conn.execute(
        """
        DELETE FROM edwarddouglas_substack_ingest_issues
...
def post_already_parsed(conn: Any, source_post_id: str) -> bool:
    row = conn.execute(
        """
        SELECT 1
        FROM edwarddouglas_substack_posts
...
def discover_rss_posts(fetcher: CachedFeedFetcher, args: argparse.Namespace) -> list[tuple[theory.PostRecord, Path, str]]:
    print(f"Reading Edward Douglas Substack RSS {args.feed_url}", file=sys.stderr)
...
def discover_archive_posts(fetcher: CachedFeedFetcher, args: argparse.Namespace) -> list[tuple[theory.PostRecord, Path, str]]:
    ...
    while not done:
        ...
        print(f"Reading Edward Douglas Substack archive page {page} {url}", file=sys.stderr)
        ...
        for item in items:
            ...
            print(f"Reading Edward Douglas Substack post {slug}", file=sys.stderr)
...
def import_parse_result(
    ...
    *,
    ...
    predictions = theory.match_predictions(
        conn,
        result.predictions,
        source_key=movie_identity.SOURCE_EDWARD_DOUGLAS_SUBSTACK,
    )
    ...
    if not predictions and theory.looks_like_forecast_post(result.post):
        insert_issue(
            ...
            post_url=result.post.post_url,
            source_movie_title=None,
            details=f"No Weekend Warrior prediction list parsed from {result.post.title}",
        )
    conn.execute(
        "DELETE FROM edwarddouglas_substack_predictions WHERE post_id = %s",
        (post_id,),
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(
            f"Imported {imported_posts} Edward Douglas Substack posts and {imported_predictions} predictions; "
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Import Edward Douglas Substack movie box office predictions.")
    ...
    parser.add_argument("--issue-source", default="edwarddouglas_substack_prediction_import")
    return parser
