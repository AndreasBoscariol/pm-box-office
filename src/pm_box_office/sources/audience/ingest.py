...
def initialize_database(conn: Any) -> None:
    ...
    conn.executescript(
        ...
        ALTER TABLE the_numbers_release_schedule
        ...
        WITH dates AS (
            SELECT src.movie_id, ims.snapshot_date
            FROM movie_source_ids src
            JOIN imdb_title_snapshots ims ON ims.tconst = src.source_movie_id
            WHERE src.source = 'imdb'
              AND src.match_status IN ('matched', 'manual_override')
            UNION
            SELECT src.movie_id, lfs.snapshot_date
            FROM movie_source_ids src
            JOIN letterboxd_film_snapshots lfs ON lfs.letterboxd_slug = src.source_movie_id
            WHERE src.source = 'letterboxd'
              AND src.match_status IN ('matched', 'manual_override')
        ),
        base AS (
            SELECT
            ...
            FROM dates d
            LEFT JOIN movie_source_ids imdb_src
              ON imdb_src.movie_id = d.movie_id
             AND imdb_src.source = 'imdb'
             AND imdb_src.match_status IN ('matched', 'manual_override')
            LEFT JOIN imdb_title_snapshots ims
              ON ims.tconst = imdb_src.source_movie_id
             AND ims.snapshot_date = d.snapshot_date
            LEFT JOIN movie_source_ids letterboxd_src
              ON letterboxd_src.movie_id = d.movie_id
             AND letterboxd_src.source = 'letterboxd'
             AND letterboxd_src.match_status IN ('matched', 'manual_override')
            LEFT JOIN letterboxd_film_snapshots lfs
              ON lfs.letterboxd_slug = letterboxd_src.source_movie_id
...
def upsert_movies_from_release_schedule(conn: Any, rows: list[TheNumbersReleaseRow]) -> None:
    for row in rows:
        ...
        conn.execute(
            """
            UPDATE the_numbers_release_schedule
            SET movie_id = %s
            WHERE movie_url = %s
            """,
            (movie_id, row.movie_url),
        )
...
def load_movie_id_for_imdb_tconst(conn: Any, tconst: str) -> int | None:
    return movie_identity.find_movie_id_by_source(
        conn,
        source=movie_identity.SOURCE_IMDB,
        source_movie_id=tconst,
    )
...
def upsert_imdb_match(conn: Any, match: ImdbMatch) -> bool:
    ...
    if stored_match.tconst and stored_match.match_status in {"matched", "manual_override"}:
        movie_identity.upsert_movie_source_id(
            ...
            source_title=None,
            match_status=stored_match.match_status,
...
def existing_match_status(conn: Any, *, source: str, movie_id: int) -> str | None:
    if relation_exists(conn, "movie_source_ids"):
        row = conn.execute(
            """
            SELECT match_status
...
def load_movie_id_for_letterboxd_slug(conn: Any, letterboxd_slug: str) -> int | None:
    return movie_identity.find_movie_id_by_source(
        conn,
        source=movie_identity.SOURCE_LETTERBOXD,
        source_movie_id=letterboxd_slug,
    )
...
def load_movie_imdb_tconst(conn: Any, movie_id: int) -> str | None:
    row = conn.execute(
        ...
        LIMIT 1
        """,
        (movie_id, movie_identity.SOURCE_IMDB),
    ).fetchone()
...
def load_movie_letterboxd_slug(conn: Any, movie_id: int) -> str | None:
    row = conn.execute(
        ...
        LIMIT 1
        """,
        (movie_id, movie_identity.SOURCE_LETTERBOXD),
    ).fetchone()
    return row[0] if row else None
