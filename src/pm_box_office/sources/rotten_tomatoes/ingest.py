...
def upsert_movie_match(conn: Any, match: MovieMatch) -> None:
    if match.ems_id and match.match_status in {"matched", "manual_override"}:
        conn.execute(
            ...
            """,
            (
                movie_identity.SOURCE_ROTTEN_TOMATOES,
                match.movie_id,
                match.ems_id,
            ),
        )
        row = conn.execute(
            "SELECT title FROM rotten_tomatoes_media WHERE ems_id = %s",
            (match.ems_id,),
        ).fetchone()
        movie_identity.upsert_movie_source_id(
            conn,
            movie_id=match.movie_id,
            source=movie_identity.SOURCE_ROTTEN_TOMATOES,
            source_movie_id=match.ems_id,
            source_title=str(row[0]) if row and row[0] is not None else None,
