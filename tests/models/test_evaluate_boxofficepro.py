    ...
    def test_summary_metrics_include_midpoint_errors_correlation_and_interval_coverage(self) -> None:
        rows = [
            evaluate_boxofficepro.ForecastActualRow(
                ...
                published_date=evaluate_boxofficepro.dt.date(2026, 7, 1),
            ...
            evaluate_boxofficepro.ForecastActualRow(
                ...
                published_date=evaluate_boxofficepro.dt.date(2026, 6, 28),
                target_start_date=evaluate_boxofficepro.dt.date(2026, 7, 3),
                target_end_date=evaluate_boxofficepro.dt.date(2026, 7, 5),
                lead_days=5,
                lead_bucket="3_7d",
            ...
            evaluate_boxofficepro.ForecastActualRow(
                ...
                source_context="legacy_forecast_table",
                published_date=evaluate_boxofficepro.dt.date(2026, 6, 19),
                target_start_date=evaluate_boxofficepro.dt.date(2026, 7, 3),
                target_end_date=evaluate_boxofficepro.dt.date(2026, 7, 5),
                lead_days=14,
                lead_bucket="8_14d",
        ...
        self.assertAlmostEqual(0.7514285714285714, summary.r2_gross or 0.0)
        self.assertAlmostEqual((0 + 400 + 2500) / 3, summary.mse_usd)
        ...
        self.assertAlmostEqual(0.9, summary.accuracy_pct or 0.0)
        self.assertAlmostEqual(10.0, summary.mean_signed_error_usd)
        self.assertAlmostEqual(2 / 3, summary.interval_coverage)
        self.assertEqual(["0_2d", "3_7d", "8_14d"], [row.lead_bucket for row in rows])
        self.assertEqual("after_open", evaluate_boxofficepro.lead_bucket_for_days(-1))
...
def seed_schema(conn) -> None:  # type: ignore[no-untyped-def]
    conn.executescript(
        ...
        INSERT INTO boxofficepro_articles (article_url, title, discovered_date)
        VALUES
            ('https://www.boxofficepro.com/complete/', 'Weekend Preview Complete', '2026-07-01'),
            ('https://www.boxofficepro.com/second/', 'Weekend Preview Second', '2026-06-28');
